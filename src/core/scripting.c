...
static void _mScriptCoreRunFrames(struct mCore* core, uint32_t frames) {
	/* Lua and the generic scripting bridge can use this to batch deterministic
	 * waits without paying the language-boundary cost once per frame. */
	while (frames--) {
		core->runFrame(core);
	}
}

static void _mScriptCoreRunFramesWithKeys(struct mCore* core, uint32_t keys, uint32_t frames) {
	/* Match the Qt Python helper: install the exact mask once, then keep the
	 * whole fixed wait inside native code. The mask intentionally remains
	 * active afterward so scripts can chain another segment without resending
	 * the same input. */
...
static void _mScriptCorePulseKeys(struct mCore* core, uint32_t keys, uint32_t frames) {
	/* Pulse-style route segments should not leak input into the next segment.
	 * Save the old mask and restore it after the batched wait completes. */
	const uint32_t previousKeys = core->getKeys(core);
	core->setKeys(core, keys);
	while (frames--) {
		core->runFrame(core);
	}
	core->setKeys(core, previousKeys);
}
...
// Local fork addition:
// - `runFrames(...)` keeps long waits inside native code
// - `runFramesWithKeys(...)` applies one exact mask for a batch
// - `pulseKeys(...)` restores the prior mask after the batch
//
// These methods exist so higher-level Lua/Python tooling can express
// deterministic "advance N frames with this exact input mask" workflows without
// crossing the script boundary once per frame.
mSCRIPT_DECLARE_STRUCT_VOID_METHOD(mCore, runFrames, _mScriptCoreRunFrames, 1, U32, frames);
mSCRIPT_DECLARE_STRUCT_VOID_METHOD(mCore, runFramesWithKeys, _mScriptCoreRunFramesWithKeys, 2, U32, keys, U32, frames);
mSCRIPT_DECLARE_STRUCT_VOID_METHOD(mCore, pulseKeys, _mScriptCorePulseKeys, 2, U32, keys, U32, frames);
...
mSCRIPT_DEFINE_STRUCT(mCore)
	...
	mSCRIPT_DEFINE_DOCSTRING("Run several frames in one native call")
	mSCRIPT_DEFINE_STRUCT_METHOD(mCore, runFrames)
	mSCRIPT_DEFINE_DOCSTRING("Set an exact key mask and run several frames in one native call")
	mSCRIPT_DEFINE_STRUCT_METHOD(mCore, runFramesWithKeys)
	mSCRIPT_DEFINE_DOCSTRING("Hold an exact key mask for several frames, then restore the previous key mask")
	mSCRIPT_DEFINE_STRUCT_METHOD(mCore, pulseKeys)
