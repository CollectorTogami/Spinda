...
struct mScriptEngineContext* _luaCreate(struct mScriptEngine2* engine, struct mScriptContext* context) {
	...
	status = luaL_dostring(luaContext->lua, _inputTapeLuaSource);
	if (status) {
		mLOG(SCRIPT, ERROR, "Error in dostring while initializing inputTape: %s\n", lua_tostring(luaContext->lua, -1));
		lua_pop(luaContext->lua, 1);
	} else {
		mScriptEngineSetDocstring(&luaContext->d, "inputTape",
			"Built-in Lua input-tape helper library for the local mgba-input-tape-v1 format.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.FORMAT",
			"Canonical on-disk format marker for inputTape JSON files.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.BUTTON_BITS",
			"Table of canonical GBA button names to raw bit indices used by inputTape masks.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.formatMask",
			"Format one raw input-tape mask as 0x########.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.maskFromButtons",
			"Convert a button list or A+START-style expression into one raw input-tape mask.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.buttonNames",
			"Expand one raw input-tape mask into canonical button names.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.fromRuns",
			"Build and validate a canonical input-tape table from run objects.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.fromFrames",
			"Compress exact per-frame masks into a canonical input-tape table.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.load",
			"Read, decode, and validate one mgba-input-tape-v1 JSON file.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.save",
			"Write one canonical input-tape table to JSON on disk.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.replay",
			"Replay one input tape through emu with stale-key clearing and frame-count verification by default.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.recordPlan",
			"Drive one run list through emu, capture start/end probes, and return the canonical recorded tape.");
		mScriptEngineSetDocstring(&luaContext->d, "inputTape.recordCurrentKeys",
			"Sample emu:getKeys() once per frame while advancing emulation and return a compressed tape.");
	}
