...
class CLIDebugger(NativeDebugger):
    ...
    def printf(self, message, *args, **kwargs):
        ...
        # The native backend expects `(backend, fmt, message)`, not just the
        # format string and payload. Pass the backend pointer explicitly so the
        # local SDL debugger print path works reliably.
        fmt = ffi.cast("const char *", ffi.new("char []", b"%s"))
        encoded = ffi.cast("const char *", ffi.new("char []", message.encode("utf-8")))
        self._cli.backend.printf(self._cli.backend, fmt, encoded)
