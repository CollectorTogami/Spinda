...
class Core(object):
    ...
    def load_rom(self, vfile):
        res = bool(self._core.loadROM(self._core, vfile.handle))
        if res:
            # After the core claims the ROM handle, Python should not try to
            # close it again during teardown. This mirrors the BIOS/save paths
            # and keeps file-backed examples stable on Windows.
            vfile._claimed = True
        return res
    ...
    @protected
    def get_keys(self):
        # Match the generic scripting `getKeys()` surface so shared helpers can
        # sample exact held GBA-button masks without reaching into memory.
        return int(self._core.getKeys(self._core))
