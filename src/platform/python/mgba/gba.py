...
class GBA(Core):
    ...
    def detach_sio(self, mode=None):
        # The example and test paths may attach custom SIO drivers for a short
        # time. Detach them explicitly so object destruction does not race the
        # underlying core teardown.
        if mode is None:
            modes = tuple(self._sio)
        else:
            modes = (mode,)

        for attached_mode in modes:
            lib.GBASIOSetDriver(ffi.addressof(self._native.sio), ffi.NULL, attached_mode)
            self._sio.discard(attached_mode)

    def __del__(self):
        try:
            # Destructors should be best-effort only. Avoid surfacing teardown
            # noise if the interpreter is already unwinding.
            self.detach_sio()
        except Exception:
            pass
