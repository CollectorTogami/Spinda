...
void mGLWidget::paintGL() {
	...
	// The frame counter and input viewer are independent overlays, so keep
	// painting when either is enabled even if transient OSD messages are off.
	if ((m_showOSD || m_showFrameCounter || m_showInputDisplay) && m_messagePainter) {
...
void DisplayGL::startDrawing(std::shared_ptr<CoreController> controller) {
	...
	showInputDisplay(isShowInputDisplay());
	filter(isFiltered());
...
void DisplayGL::showFrameCounter(bool enable) {
	...
	if (m_gl) {
		m_gl->setShowFrameCounter(enable);
	}
	QMetaObject::invokeMethod(m_painter.get(), "showFrameCounter", Q_ARG(bool, enable));
}

void DisplayGL::showInputDisplay(bool enable) {
	Display::showInputDisplay(enable);
	if (m_gl) {
		m_gl->setShowInputDisplay(enable);
	}
	QMetaObject::invokeMethod(m_painter.get(), "showInputDisplay", Q_ARG(bool, enable));
}
...
void PainterGL::showInputDisplay(bool enable) {
...
void PainterGL::performDraw() {
	...
	// Match the Qt paint path: the dedicated overlays stay visible even when the
	// generic "show OSD messages" toggle is off.
	if ((m_showOSD || m_showFrameCounter || m_showInputDisplay) && m_messagePainter && m_paintDev && !glContextHasBug(OpenGLBug::IG4ICD_CRASH)) {
