...
public:
	...
	bool fastForwardForced() const { return m_fastForwardForced; }
	float fastForwardRatio() const { return m_fastForwardRatio; }
	bool audioSyncBlocked() const { return m_audioSyncBlocked; }
	bool audioKillswitchEnabled() const { return m_audioKillswitch; }
	bool noRenderModeEnabled() const { return m_noRenderMode; }
	bool scriptTimingOverride() const { return m_scriptTimingOverride; }
	bool savestateCacheEnabled() const { return m_savestateCacheEnabled; }
	int savestateCacheMaxEntries() const { return m_savestateCacheMaxEntries; }
	quint64 savestateCacheMaxBytes() const { return m_savestateCacheMaxBytes; }
	quint64 savestateCacheUsedBytes() const;
	int savestateCacheEntryCount() const;
	bool videoLayerEnabled(size_t id) const;
	QString currentSaveSuffix() const;
	QString currentSaveFileName() const;

	void addFrameAction(std::function<void ()> callback);
	uint64_t frameCounter() const { return m_frameCounter; }
	uint32_t currentKeys() const { return m_currentKeys; }
	uint32_t lastFrameKeys() const { return m_lastFrameKeys; }
	uint32_t pendingKeys();
...
public slots:
	...
	void setFastForwardRatio(float ratio);

	void changePlayer(int id);

	void overrideMute(bool);
	void setAudioSyncBlocked(bool blocked);
	void setAudioKillswitch(bool);
	void setNoRenderMode(bool);
	void setScriptTimingOverride(bool);
	void configureSavestateMemoryCache(bool enabled, int maxEntries, quint64 maxBytes);
	...
	void importSave(const QString& path);
	void exportSave(const QString& path);
	...
	void pressKeyForFrames(int key, int frames);
	void setAutofire(int key, bool enable);
	void setInputTapeOverride(bool enable, uint32_t keys = 0);
...
private:
	void updateKeys();
	void installPendingKeysForBoundedStep();
	int updateAutofire();
	void finishFrame();
	void applyNoRenderMode();
	int resolvedSavePlayerId() const;
	VFile* openCurrentSave(int mode) const;
	QString savestateCacheKeyForSlot(int slot) const;
	QString savestateCacheKeyForPath(const QString& path) const;
	...
	int m_activeKeys = 0;
	int m_removedKeys = 0;
	uint32_t m_currentKeys = 0;
	uint32_t m_lastFrameKeys = 0;
	// Native input-tape replay needs exact per-frame masks. While this override
	// is active, updateKeys() ignores live keyboard/controller polling so a held
	// host key cannot contaminate deterministic tape playback.
	bool m_inputTapeOverride = false;
	uint32_t m_inputTapeKeys = 0;
	// Virtual Pad timed presses count down in the frame loop instead of
	// rescheduling one lambda per key per frame.
	int m_timedKeyFrames[32] = {};
	bool m_autofire[32] = {};
	...
	bool m_audioSyncBlocked = false;
	bool m_audioKillswitch = false;
	bool m_noRenderMode = false;
	bool m_scriptTimingOverride = false;
	bool m_savestateCacheEnabled = false;
	int m_savestateCacheMaxEntries = 64;
	quint64 m_savestateCacheMaxBytes = 4ull * 1024ull * 1024ull * 1024ull;
	QVector<bool> m_savedVideoLayers;
	std::unique_ptr<SavestateMemoryCache> m_savestateCache;
