...
void SettingsView::updateConfig() {
	...
	saveSetting("showInputDisplay", m_ui.showInputDisplay);
	...
	bool darkMode = m_ui.darkMode->isChecked();
	if (darkMode != m_controller->getQtOption("darkMode").toBool()) {
		// Apply the theme immediately so UI automation users can confirm the
		// setting visually without restarting the emulator.
		m_controller->setQtOption("darkMode", darkMode);
		GBAApp::app()->setDarkMode(darkMode);
	}
...
void SettingsView::reloadConfig() {
	...
	loadSetting("showInputDisplay", m_ui.showInputDisplay);
	...
	// Qt-only setting stored alongside the normal config options.
	m_ui.darkMode->setChecked(m_controller->getQtOption("darkMode").toBool());
