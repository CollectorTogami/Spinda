...
void applyMenuTheme(QMenuBar* menubar) {
	if (!menubar) {
		return;
	}
	// On Windows the menu bar can keep the old light chrome unless it is marked
	// as a styled widget explicitly. Re-apply the app palette on each rebuild so
	// dark mode reaches the visible File/Emulation/etc strip too.
