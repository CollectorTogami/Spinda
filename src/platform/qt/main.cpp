...
bool spindaNativePhase3Headless() {
	if (!qEnvironmentVariableIsSet("MGBA_SPINDA_NATIVE_PHASE3_AUTORUN")) {
		return false;
	}
	const QByteArray raw = qgetenv("MGBA_SPINDA_NATIVE_PHASE3_HEADLESS").trimmed().toLower();
	return raw == QByteArrayLiteral("1")
		|| raw == QByteArrayLiteral("true")
		|| raw == QByteArrayLiteral("yes")
		|| raw == QByteArrayLiteral("on");
}
