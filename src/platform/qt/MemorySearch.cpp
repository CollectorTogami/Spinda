...
void MemorySearch::refresh() {
	...
	for (size_t i = 0; i < mCoreMemorySearchResultsSize(&m_results); ++i) {
		...
		QTableWidgetItem* address = new QTableWidgetItem(QString("%1").arg(result->address, 8, 16, QChar('0')));
		m_ui.results->setItem(i, 0, address);
		...
		if (result->type == mCORE_MEMORY_SEARCH_INT && m_ui.numHex->isChecked()) {
			...
			case mCORE_MEMORY_SEARCH_GUESS:
				// Guess searches normally collapse to integer hits today. Keep
				// this defensive path so future guess-typed rows still render.
				item = new QTableWidgetItem(QString::number(result->oldValue));
				break;
			}
		}
		if (!item) {
		...
		case mCORE_MEMORY_SEARCH_STRING:
			...
			break;
		case mCORE_MEMORY_SEARCH_GUESS:
			type = new QTableWidgetItem(tr("guess%1").arg(divisor));
			break;
		}
		if (!type) {
			type = new QTableWidgetItem(tr("unknown"));
		}
