...
#else
/* This stdin/stdout fallback is intentionally simple. It keeps the local
 * Python-enabled debugger usable when the richer histedit line editor is not
 * present, which is enough for scripted emulator control and debugging. */
static void _breakIntoDefault(int signal) {
	UNUSED(signal);
	mDebuggerEnter(&_activeDebugger->d, DEBUGGER_ENTER_MANUAL, 0);
}
...
const char* _CLIDebuggerEditLineReadLine(struct CLIDebuggerBackend* be, size_t* len) {
	struct CLIDebuggerEditLineBackend* elbe = (struct CLIDebuggerEditLineBackend*) be;
	if (!fgets(elbe->line, sizeof(elbe->line), stdin)) {
		*len = 0;
		return NULL;
	}

	*len = strlen(elbe->line);
	if (*len == 1 && elbe->line[0] == '\n') {
		return elbe->line;
	}
	if (*len > 0 && elbe->line[*len - 1] == '\n') {
		--*len;
	}
	return elbe->line;
}
...
const char* _CLIDebuggerEditLineHistoryLast(struct CLIDebuggerBackend* be, size_t* len) {
	...
	*len = strlen(elbe->lastLine);
	return elbe->lastLine;
}

void _CLIDebuggerEditLineHistoryAppend(struct CLIDebuggerBackend* be, const char* line) {
	struct CLIDebuggerEditLineBackend* elbe = (struct CLIDebuggerEditLineBackend*) be;
	snprintf(elbe->lastLine, sizeof(elbe->lastLine), "%s", line);
}
